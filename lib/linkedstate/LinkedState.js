...
var LinkedState = /** @class */ (function () {
  ...
  LinkedState.prototype._unobserve = function (setState) {
    this._observeList = this._observeList.filter(function (it) { return it !== setState; });
  };
  return LinkedState;
}());
exports.LinkedState = LinkedState;
function useLinkedState(linkedState) {
  var _a = react_1.useState(linkedState.state()), state = _a[0], setLocalState = _a[1];
...
function useComputedState(rLinkState, depList) {
  if (depList === void 0) { depList = []; }
  var _a = react_1.useState(rLinkState.state()), state = _a[0], setLocalState = _a[1];
  react_1.useEffect(function () {
    rLinkState._observe(setLocalState);
    return function () { return rLinkState._unobserve(setLocalState); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  return state;
}
